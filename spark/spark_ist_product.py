...
def prequery_df_crt_product(df: DataFrame, date_str: str) -> DataFrame:
    df_output = df.filter(
        (F.col("cob_dt") == date_str)
    )
    return df_output
    
def prequery_df_ist_product(df: DataFrame, date: date) -> DataFrame:
    df_output = df.filter(
        (F.col("cob_dt") == date - 1)
    )
    return df_output
...
def main():
    ...
    # set data source
    df_crt_product_current = spark.table("crt_product_current")
    df_ist_product_current = spark.table("ist_product_current")

    # prequery data source and transform
    df_crt_product_current = prequery_df_crt_product(
        df_crt_product_current, 
        data_date_str
    )
    df_ist_product_current = prequery_df_ist_product(
        df_ist_product_current,
        date
    )
    ...
    # write in database
    writer_scd4(spark, df_crt_product_current, df_delete, df_update, 'ist_product')
    spark.stop()
