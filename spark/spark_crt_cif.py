...
def prequery_df_stg_cif(df: DataFrame, date_str: str) -> DataFrame:
    df_output = df.filter(
        (F.col("cob_dt") == date_str)
    )
    return df_output
    
def prequery_df_crt_cif(df: DataFrame, date: date) -> DataFrame:
    df_output = df.filter(
        (F.col("cob_dt") == date - 1)
    )
    return df_output
...
def main():
    ...
    # set data source
    df_stg_prf_cif = spark.table("stg_prf_cif")
    df_ctr_cif_current = spark.table("ctr_cif_current")

    # prequery data source and transform
    df_stg_prf_cif = prequery_df_stg_cif(
        df_stg_prf_cif, 
        data_date_str
    )
    df_ctr_cif_current = prequery_df_crt_cif(
        df_ctr_cif_current,
        date
    )
    ...
    # write in database
    writer(spark, df_stg_prf_cif, df_delete, df_update, 'crt_cif')
    spark.stop()
