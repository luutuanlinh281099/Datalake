...
file_path = 'lake/spark/spark_ist_dep_smy'
...
with DAG('dag_ist_dep_smy',
    ...
    consume_kafka = KafkaConsumerOperator(
        ...
        on_message_callback=consume_kafka_messages('mesage_ist_dep_smy'),
    )
        ...
        sql="""
            SELECT COUNT(*) FROM (
                ...
                AND TABLE_NAME IN (
                    ...
                    FROM TABLE_DEPEN
                    WHERE TABLE_NAME_AFTER = 'ist_dep_smy'))
                EXCEPT
                (SELECT TABLE_NAME_BEFORE
                FROM TABLE_DEPEN
                WHERE TABLE_NAME_AFTER = 'ist_dep_smy')
    ...
    trino_query = TrinoOperator(
        ...
        sql="""
            SELECT TABLE_NAME_AFTER FROM TABLE_DEPEN WHERE TABLE_NAME_BEFORE = 'ist_dep_smy'
        """,
