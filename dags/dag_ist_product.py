...
file_path = 'lake/spark/spark_ist_product'
...
with DAG('dag_ist_product',
    ...
    consume_kafka = KafkaConsumerOperator(
        ...
        on_message_callback=consume_kafka_messages('mesage_ist_product'),
    )
        ...
        sql="""
            SELECT COUNT(*) FROM (
                ...
                AND TABLE_NAME IN (
                    ...
                    FROM TABLE_DEPEN
                    WHERE TABLE_NAME_AFTER = 'ist_product'))
                EXCEPT
                (SELECT TABLE_NAME_BEFORE
                FROM TABLE_DEPEN
                WHERE TABLE_NAME_AFTER = 'ist_product')
    ...
    trino_query = TrinoOperator(
        ...
        sql="""
            SELECT TABLE_NAME_AFTER FROM TABLE_DEPEN WHERE TABLE_NAME_BEFORE = 'ist_product'
        """,
