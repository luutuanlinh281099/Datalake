...
file_path = 'lake/spark/spark_crt_dep_dtls'
...
with DAG('dag_crt_dep_dtls',
    ...
    consume_kafka = KafkaConsumerOperator(
        ...
        on_message_callback=consume_kafka_messages('mesage_crt_dep_dtls'),
    )
        ...
        sql="""
            SELECT COUNT(*) FROM (
                ...
                AND TABLE_NAME IN (
                    ...
                    FROM TABLE_DEPEN
                    WHERE TABLE_NAME_AFTER = 'crt_dep_dtls'))
                EXCEPT
                (SELECT TABLE_NAME_BEFORE
                FROM TABLE_DEPEN
                WHERE TABLE_NAME_AFTER = 'crt_dep_dtls')
    ...
    trino_query = TrinoOperator(
        task_id='trino_query',
        name='trino_query'
        trino_conn_id='datalake',
        sql="""
            SELECT TABLE_NAME_AFTER FROM TABLE_DEPEN WHERE TABLE_NAME_BEFORE = 'crt_dep_dtls'
        """,
