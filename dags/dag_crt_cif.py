...
file_path = 'lake/spark/spark_crt_cif'
...
with DAG('dag_crt_cif',
    ...
    consume_kafka = KafkaConsumerOperator(
        ...
        on_message_callback=consume_kafka_messages('mesage_crt_cif'),
    )
        ...
        sql="""
            SELECT COUNT(*) FROM (
                ...
                AND TABLE_NAME IN (
                    ...
                    FROM TABLE_DEPEN
                    WHERE TABLE_NAME_AFTER = 'crt_cif'))
                EXCEPT
                (SELECT TABLE_NAME_BEFORE
                FROM TABLE_DEPEN
                WHERE TABLE_NAME_AFTER = 'crt_cif')
    ...
    trino_query = TrinoOperator(
        ...
        sql="""
            SELECT TABLE_NAME_AFTER FROM TABLE_DEPEN WHERE TABLE_NAME_BEFORE = 'crt_cif'
        """,
